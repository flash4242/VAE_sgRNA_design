...
for epoch in range(config.epochs):
    ...
    for inputs, targets in train_loader:
        ...
        vae_loss_value = vae_loss(reconstructed, targets, z_mean, z_log_var)

        # Generate sgRNAs from latent space and compute on-target loss
        with torch.no_grad():
            z = torch.randn(config.generated_sgrnas, config.latent_dim).to(device)
            ...
            for sgRNA in generated_sgRNAs:
                predicted_efficacy = predict_sgRNA_eff(ontarget_model, sgRNA)
                total_ontarget_loss += on_target_loss(predicted_efficacy)
            total_ontarget_loss /= config.generated_sgrnas

        
        # Scale training on-target loss
        total_ontarget_loss *= config.ontarget_loss_weight
        # Combine losses
        loss = vae_loss_value + total_ontarget_loss
        loss.backward()
        optimizer.step()

        train_vae_loss += vae_loss_value.item()
        train_ontarget_loss += total_ontarget_loss
    ...
    train_ontarget_losses.append(train_ontarget_loss)
    ...
    with torch.no_grad():
        for inputs, targets in val_loader:
            ...
            # Generate 5 sgRNAs for validation on-target loss
            z = torch.randn(config.generated_sgrnas, config.latent_dim).to(device)
            generated_outputs = model.decode(z)
            generated_indices = torch.argmax(generated_outputs, dim=-1)
            ...
            # Scale validation ontarget loss
            total_ontarget_loss *= config.ontarget_loss_weight

            val_vae_loss += vae_loss_value.item()
            val_ontarget_loss += total_ontarget_loss
            val_total_loss += vae_loss_value.item() + total_ontarget_loss
    ...
    val_ontarget_losses.append(val_ontarget_loss)
